...
Every app here talks to [Ollama](https://ollama.com), which runs the models on the GPU. On top of it:

- [T3 Code](https://t3.codes) ([source](https://github.com/pingdotgg/t3code)), an open-source
  desktop app for running coding agents. Here it runs [OpenCode](https://opencode.ai), an
  open-source coding agent, against the local models.
- [Jan](https://jan.ai), a desktop chat app that can use tools.
- [AnythingLLM](https://anythingllm.com), which answers questions from your own documents, with citations.
- VS Code with [Twinny](https://github.com/twinnydotdev/twinny), for autocomplete only.
...
![The expense workbook saved by qwen2.5:14b, with the Travel utilization cell outlined in red and showing #DIV/0!](/blog/local-ai-on-a-12gb-gpu/div0-reported-as-na.png "The workbook qwen2.5:14b saved, drawn from the file itself. Its report said Travel utilization now shows “n.a.”; the saved cell still shows #DIV/0!. The red outline is mine.")
...
![OpenCode in a terminal, where the model prints two tool calls as JSON text with /home/user paths and nothing runs](/blog/local-ai-on-a-12gb-gpu/tool-calls-as-text.png "August, OpenCode with hhao/qwen2.5-coder-tools: the tool calls come out as JSON text, so nothing runs, and the paths are Linux paths on a Windows machine. That model is no longer in my setup.")
...
![Jan chat window showing Generation failed, Forbidden, under a user message](/blog/local-ai-on-a-12gb-gpu/jan-forbidden.png "The 403 in Jan, from my first setup in August, before I changed OLLAMA_ORIGINS.")
...
**2. Models.** Download three models ([qwen3.5](https://ollama.com/library/qwen3.5),
[qwen3-coder](https://ollama.com/library/qwen3-coder),
[qwen2.5-coder](https://ollama.com/library/qwen2.5-coder)), then create two 32K variants:
...
- **Other runtimes.** I did not compare Ollama with [LM Studio](https://lmstudio.ai) or
  [llama.cpp](https://github.com/ggml-org/llama.cpp)'s own server. Every app here could talk to
...
I'm Akash Damle, founder of Matalli Infotech Private Limited, an early-stage company I'm building
from the ground up. I write to mark milestones: what was built, what broke, and what it taught me.

If you follow this setup, especially on different hardware, I would like to hear how it went: what
you ran, what happened, and what you changed. Disagreement is as welcome as agreement, as long as it
comes with a reason. Reports like that are how the next version of this guide gets better.
